...
prompt drop job
begin dbms_scheduler.stop_job ('mailgun_process_queue'); exception when others then if sqlcode != -27366 then raise; end if; end;
/
begin dbms_scheduler.drop_job('mailgun_process_queue'); exception when others then if sqlcode!=-27475 then raise; end if; end;
/
...
/
begin dbms_aqadm.drop_queue_table (user||'.mailgun_queue_tab'); exception when others then if sqlcode!=-24010 then raise; end if; end;
/
